...
import {
    ...
    MAX_FILE_SIZE_BYTES,
    type UploadOriginType,
} from '@/lib/chat/file-upload-utils';
...
const useUploadFiles = (agent?: AgentType, originType?: UploadOriginType) => {
    ...
    const uploadArgsRef = useRef<
        Map<string, { file: File; groupName?: string; tagToField?: string; tagToId?: string }>
    >(new Map());
    ...
    const onUploadFile = async (
        ...
        try {
            ...
            uploadArgsRef.current.delete(tempId);
        } catch (error) {
    ...
    const onChangeFile = (
        ...
        try {
            ...
            if (!isMultipleAllowed && validFiles.length > 0 && mappedFiles.length > 0) {
                const existingFiles = groupName
                    ? uploadFiles.files.filter((f: FileType) => f.groupName !== groupName)
                    : [];
                ...
                if (firstFile && firstRawFile) {
                    ...
                    uploadArgsRef.current.set(firstFile.tempId!, {
                        file: firstRawFile,
                        groupName,
                        tagToField,
                        tagToId,
                    });
                    void onUploadFile(firstRawFile, firstFile.tempId!, groupName, tagToField, tagToId);
                }
            } else {
                actions.setFiles([...uploadFiles.files, ...mappedFiles]);
                validFiles.forEach((file, index) => {
                    const tempId = mappedFiles[index].tempId!;

                    uploadArgsRef.current.set(tempId, {
                        file,
                        groupName,
                        tagToField,
                        tagToId,
                    });
                    void onUploadFile(file, tempId, groupName, tagToField, tagToId);
                });
            }
        } catch (err) {
    ...
    const retryUpload = (tempId: string) => {
        const stored = uploadArgsRef.current.get(tempId);
        ...
        void onUploadFile(stored.file, tempId, stored.groupName, stored.tagToField, stored.tagToId);
    };

    return { onChangeFile, retryUpload };
};
