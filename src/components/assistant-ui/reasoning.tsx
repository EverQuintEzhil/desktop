...
function ReasoningTrigger({
    ...
    const resolveLabel = (): string => {
        ...
        if (durationMs != null) return `Thought for ${formatElapsed(durationMs)}`;
    ...
    return (
        ...
        >
            ...
            <ChevronDownIcon
                ...
                className={cn(
                    'aui-reasoning-trigger-chevron mt-0.5 size-4',
...
function ReasoningText({ className, ...props }: React.ComponentProps<'div'>) {
    return (
        <div
            ...
            className={cn(
                'aui-reasoning-text scrollbar-controller scrollbar-vertical scrollbar-horizontal relative z-0 max-h-64 space-y-4 pt-2 pb-2 pl-6 leading-relaxed',
...
const ReasoningGroupImpl: ReasoningGroupComponent = ({ children, startIndex, endIndex }) => {
    const { isStreaming: isReasoningStreaming, elapsedMs } = useReasoningElapsed(startIndex, endIndex);
    ...
    return (
        <ReasoningRoot key={isMessageRunning ? 'reasoning-running' : 'reasoning-done'} defaultOpen={isMessageRunning}>
            <ReasoningTrigger active={isReasoningStreaming} durationMs={elapsedMs} />
